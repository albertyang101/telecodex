...
const RESULT_NOUN_COMPOUND_RE =
  /^(?:再|接着|随后|准备)(?:检查|查看|查询|查阅|核查|测试)[\p{Script=Han}]{0,6}(?:结果|报告|任务)(?=(?:显示|表明|说明|证明|已|为|是|未|仍|依然|成功|失败|完成|[：:]))/u;
...
function recoverableIntermediateUpdate(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const resultClause = head.match(
      /(?:^|[，,；;。.!：:]\s*)((?:关键发现|阶段结果|结果(?:是|为)?|发现(?!\s*[\p{Script=Han}]{0,4}(?:没有|未)(?:继续|去)?(?:跑|检查|查看|测试|读|做|处理))(?=[^，,；;。.!：:]{0,40}(?:没有|未|仍|依然|已经|是|为|成功|失败|生效|运行|完成))|查明|已完成|成功(?=了|是|为|[：:]|[。！？!?]|$)|失败(?=了|是|为|[：:]|[。！？!?]|$)|通过(?=了|是|为|[：:]|[。！？!?]|$)|根因(?=是|为|[：:])|已经|仍在)[：:]?)/u,
    );
