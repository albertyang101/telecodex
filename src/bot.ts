...
const PROCESS_NARRATION_HEAD_RE =
  /^(?:我先(?:把|将)|我先(?:去|来)?(?:看|看看|查看|查|检查|确认(?:一下|完了)?|跑|读|做|处理|准备)|我准备(?:把|将)|我准备(?:去|先)?(?:看|查看|查|检查|确认|跑|读|做|处理)|我接下来|我现在(?:要|去|来|先)|下一步|接下来|然后|再(?:继续|去|检查(?!结果)|查看|查|跑|读|做|处理|测试(?!结果))|(?:接着|随后|准备)(?:继续|去|检查(?!结果)|查看|查|跑|读|做|处理|测试(?!结果))|收到[，。]?\s*我先|let me\b|going to\b|now(?:\s+|,\s*)(?:i(?:['’]ll|\s+will|\s+am|['’]m)|going to)\b|i(?:['’]ll|\s+will|\s+am|['’]m)\b|next\b)/i;
...
function recoverableIntermediateUpdate(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const head = normalizeIntermediateUpdateHead(lines[index] ?? "");
    if (!isProcessNarrationHead(head)) {
      return lines.slice(index).join("\n").trim();
    }

    const resultClause = head.match(
      /(?:^|[，,；;。.!：:]\s*)((?:关键发现|阶段结果|结果(?:是|为)?|发现(?!\s*(?:还)?(?:没有|未)(?:跑|检查|查看|测试|读|做|处理))(?=[^，,；;。.!：:]{0,40}(?:没有|未|仍|依然|已经|是|为|成功|失败|生效|运行|完成))|查明|已完成|成功(?=了|是|为|[：:]|[。！？!?]|$)|失败(?=了|是|为|[：:]|[。！？!?]|$)|通过(?=了|是|为|[：:]|[。！？!?]|$)|根因(?=是|为|[：:])|已经|仍在)[：:]?)/,
    );
